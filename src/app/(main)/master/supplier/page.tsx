    ...
    <Modal onClose={onClose}>
      <form onSubmit={handleSubmit(onSubmit)} className="p-4">
        ...
        <div className="mt-2">
          ...
          <input {...register('idNumber')} className="border p-2 w-full rounded-md" />
        </div>
        <div className="mt-2">
        ...
        <div className="mt-2">
          ...
          <input {...register('phoneNumber')} className="border p-2 w-full rounded-md" />
        </div>
...
const SuppliersTable = ({
  ...
  const tableKey = useMemo(() => 'suppliers-table', []);
...
const SupplierPage: React.FC = () => {
  ...
  const fetchCustomers = async () => {
    ...
    try {
      const response = await fetch(
        `/api/suppliers?page=${currentPage}&limit=${itemsPerPage}&search=${searchTerm}&sortBy=${sortKey}&sortOrder=${sortDirection}`
      );
  ...
  const handleCreateSubmit = async (data: Omit<Customer, 'id'>) => {
    ...
    try {
      ...
      if (!response.ok) {
        showToast('error', 'Gagal menambahkan data supplier, KTP sudah terdaftar');
      ...
      showToast('success', 'Data supplier berhasil ditambahkan');
    } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-4">
        <Input
          placeholder="Cari nomor KTP atau nama supplier..."
          value={searchTerm}
          onChange={(e) => {
